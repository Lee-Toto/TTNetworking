...
public func bundleImage(_ name: String?) -> UIImage? {
    guard let name = name else { return nil }
    let bundle = Bundle(for: ESConfig.classForCoder())
    return UIImage(named: name, in: Bundle(path: bundle.path(forResource: "TTNetworking", ofType: "bundle") ?? ""), compatibleWith: nil)?.withRenderingMode(.alwaysOriginal)
}
